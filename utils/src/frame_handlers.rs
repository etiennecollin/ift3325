...
pub fn handle_connection_start(
    safe_window: SafeWindow,
    frame: &Frame,
    writer_tx: &UnboundedSender<Vec<u8>>,
    condition: SafeCond,
...
pub fn handle_reject(
    ...
    for (_, frame) in frames {
        ...
        // data to the writer.
        writer_tx.send(frame).expect("Failed to resend frame");
    }
...
pub fn handle_information(
    ...
    writer_tx: &UnboundedSender<Vec<u8>>,
    condition: SafeCond,
    assembler_tx: &UnboundedSender<Vec<u8>>,
    ...
    if *expected_info_num != frame.num {
        return handle_dropped_frame(&frame, safe_window, writer_tx, expected_info_num);
    }
...
/// receive.
pub fn handle_p(writer_tx: &UnboundedSender<Vec<u8>>, expected_info_num: &u8) -> bool {
